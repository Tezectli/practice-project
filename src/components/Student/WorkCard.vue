  ...
  <div class="card">
    <div class="card-header">
      <van-image
        ...
        lazy-load
        radius="10px"
        style="margint-top:5px"
      >
        <template v-slot:error>加载失败</template>
      </van-image>
    </div>
    <div class="card-title">
      <van-tag round type="primary">{{work.workType==null? ' ':work.workType}}</van-tag>
      {{work.postTitle}}
    </div>
    <div class="card-content">{{work.postContent==null? ' ':work.postContent}}</div>
    <div class="card-info">
      <span class="card-info__left">
        <van-button class="info-left__btn" plain size="mini" round @click="postLike">
          <van-icon class-prefix="zhsj-icon" :name="icon" size="20" color="red" />
